...
export default function AdminLeads() {
  ...
  const { data: leadsData, isLoading } = useQuery({
    ...
    queryFn: () => fetchLeads({
      search: search || "", status: filterStatus, source: filterSource, limit: "100",
    }),
  });
  ...
  const patchMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["leads"] });
      qc.invalidateQueries({ queryKey: ["leadStats"] });
      qc.invalidateQueries({ queryKey: ["leadDetail", detailId] });
  ...
  const activityMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["leadDetail", detailId] });
      qc.invalidateQueries({ queryKey: ["leadFollowUps"] });
  ...
  const [convertForm, setConvertForm] = useState({
    ...
    serviceId: "", scheduledDate: "", amount: "", subscriptionType: "monthly",
  });
                ...
                <div className="space-y-3 mt-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Input value={form.secondaryPhone} onChange={e => setForm({ ...form, secondaryPhone: e.target.value })} placeholder="Secondary" />
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                  ...
                  <div>
                    ...
                    <Input value={form.valueEstimate} onChange={e => setForm({ ...form, valueEstimate: e.target.value })} placeholder="Estimated value" />
                  </div>
                  <div>
                    <Label>Notes</Label>
                    <textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })}
                      className="w-full bg-background border border-border rounded-md px-3 py-2 text-sm resize-none h-20" placeholder="Initial notes..." />
                  </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span className="bg-green-500/10 text-green-400 px-2 py-1 rounded-md font-medium">
              {stats?.conversionRate ?? 0}% converted
            </span>
          </div>
        </div>
                    ...
                    {isLoading ? (
                      ...
                      items.map(lead => (
                        <button
                          ...
                          className="w-full text-left bg-card border border-border rounded-lg p-3 hover:border-primary/40 hover:shadow-sm transition-all group"
                        >
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex justify-end">
          <div className="w-full max-w-lg bg-background h-full overflow-y-auto border-l border-border shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              ...
              <button onClick={() => setDetailId(null)} className="text-muted-foreground hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {detail.serviceInterest && (
                  <Badge variant="outline" className="capitalize">{detail.serviceInterest.replace("_", " ")}</Badge>
                )}
                {detail.valueEstimate && (
                  <span className="text-primary font-medium text-sm">₹{Number(detail.valueEstimate).toLocaleString("en-IN")}</span>
                )}
              </div>
              ...
              <div className="flex gap-2 flex-wrap">
                <Button size="sm" variant="outline" onClick={() => setShowConvert(true)}>
                  <UserPlus size={13} className="mr-1.5" />Convert to Customer
                </Button>
                {detail.status !== "lost" && (
                  <Button size="sm" variant="outline" onClick={() => patchMut.mutate({ id: detail.id, body: { status: "lost" } })}>
                    <X size={13} className="mr-1.5" />Mark Lost
                  </Button>
                )}
              </div>
              ...
              {showConvert && (
                <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                  <h3 className="font-semibold text-sm flex items-center gap-2">
                    <ArrowRight size={14} className="text-primary" />Convert Lead
                  </h3>
                  <div className="space-y-2 text-sm">
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={convertForm.createCustomer} onChange={e => setConvertForm({ ...convertForm, createCustomer: e.target.checked })} />
                      Create Customer
                    </label>
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={convertForm.createBooking} onChange={e => setConvertForm({ ...convertForm, createBooking: e.target.checked })} />
                      Create Booking
                    </label>
                    <label className="flex items-center gap-2">
                      <input type="checkbox" checked={convertForm.createSubscription} onChange={e => setConvertForm({ ...convertForm, createSubscription: e.target.checked })} />
                      Create Subscription
                    </label>
                  </div>
                  ...
                  {convertForm.createSubscription && (
                    <select value={convertForm.subscriptionType} onChange={e => setConvertForm({ ...convertForm, subscriptionType: e.target.value })}
                      className="w-full bg-background border border-border rounded-md px-2 py-1.5 text-sm">
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" className="bg-primary text-secondary" onClick={() => convertMut.mutate({ id: detail.id, body: convertForm })}>
              ...
              <div>
                <h3 className="font-semibold text-sm mb-3 flex items-center gap-2">
                  <MessageSquare size={14} className="text-primary" />Activity Timeline
                </h3>
              ...
              <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                ...
                <textarea value={newNote} onChange={e => setNewNote(e.target.value)}
                  className="w-full bg-background border border-border rounded-md px-3 py-2 text-sm resize-none h-20" placeholder="Write a note..." />
                <div className="flex items-center gap-3">
                  <div className="flex items-center gap-2 text-sm">
                    <Calendar size={13} className="text-muted-foreground" />
                    <Input type="datetime-local" value={newFollowUp} onChange={e => setNewFollowUp(e.target.value)} className="w-48 text-sm" />
                  </div>
                  <Button size="sm" className="bg-primary text-secondary ml-auto"
                    onClick={() => {
                      activityMut.mutate({ id: detail.id, body: { type: "note", body: newNote, followUpAt: newFollowUp } });
                      setNewNote(""); setNewFollowUp("");
                    }}>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-lg bg-background rounded-xl border border-border shadow-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-semibold text-sm flex items-center gap-2">
                <Clock size={14} className="text-primary" />Upcoming Follow-ups
              </h3>
              <button onClick={() => setShowFollowUps(false)} className="text-muted-foreground hover:text-white">
                <X size={16} />
              </button>
            </div>
      ...
      {showStats && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-background rounded-xl border border-border shadow-2xl max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-semibold text-sm flex items-center gap-2">
                <BarChart3 size={14} className="text-primary" />Lead Analytics
              </h3>
              <button onClick={() => setShowStats(false)} className="text-muted-foreground hover:text-white">
                <X size={16} />
              </button>
            </div>
            <div className="p-4 space-y-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-card border border-border rounded-xl p-4 text-center">
                  <p className="font-bold text-2xl text-primary">{stats?.total ?? 0}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Total Leads</p>
                </div>
                <div className="bg-card border border-border rounded-xl p-4 text-center">
                  <p className="font-bold text-2xl text-green-400">{stats?.converted ?? 0}</p>
                  <p className="text-xs text-muted-foreground mt-0.5">Converted</p>
                </div>
                <div className="bg-card border border-border rounded-xl p-4 text-center">
                  <p className="font-bold text-2xl text-emerald-400">{stats?.conversionRate ?? 0}%</p>
              ...
              <div>
                <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
                  <TrendingUp size={14} className="text-primary" />By Source
                </h4>
                  ...
                  {(stats?.bySource ?? []).map(s => (
                    <div key={s.source} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-2 bg-muted rounded-full overflow-hidden">
                        <div className="h-full bg-primary rounded-full" style={{ width: `${Math.max((s.count / (stats?.total || 1)) * 100, 3)}%` }} />
                      </div>
