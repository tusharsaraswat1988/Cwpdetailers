...
interface Lead {
  id: number; name: string; phone: string; secondaryPhone?: string | null; city?: string | null;
  source: string; serviceInterest?: string | null; assignedToStaffId?: number | null; assignedToName?: string | null;
  status: LeadStatus; notes?: string | null; nextFollowUpAt?: string | null; valueEstimate?: string | null;
  lostReason?: string | null; createdAt: string; updatedAt: string;
  customerId?: number | null; bookingId?: number | null; subscriptionId?: number | null;
}
interface LeadActivity { id: number; type: string; body: string; createdAt: string; createdBy?: number | null; }
interface LeadStats { total: number; converted: number; conversionRate: number; bySource: { source: string; count: number }[]; byStatus: { status: string; count: number }[]; }
...
async function fetchLeads(params: Record<string, string> = {}): Promise<{ data: Lead[]; total: number }> {
  const qs = new URLSearchParams(params).toString(); const res = await fetch(`/api/leads?${qs}`); if (!res.ok) throw new Error("Failed to fetch leads"); return res.json();
}
...
function DraggableLeadCard({ lead, onClick }: { lead: Lead; onClick: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners}>
      <button onClick={onClick} className="w-full text-left bg-card border border-border rounded-lg p-3 hover:border-amber-500/40 hover:shadow-sm transition-all group cursor-pointer">
        <div className="flex items-center justify-between mb-1"><p className="font-medium text-sm truncate">{lead.name}</p><span className="text-xs text-muted-foreground">#{lead.id}</span></div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2"><Phone size={10} />{lead.phone}{lead.city && <span>· {lead.city}</span>}</div>
        <div className="flex items-center justify-between">
          <Badge variant="outline" className="text-[10px] h-5">{SOURCE_LABEL[lead.source] || lead.source}</Badge>
          {lead.nextFollowUpAt && <span className="flex items-center gap-1 text-[10px] text-yellow-400"><Calendar size={10} />{new Date(lead.nextFollowUpAt).toLocaleDateString("en-IN")}</span>}
        </div>
        {lead.valueEstimate && <p className="text-xs text-primary font-medium mt-1.5">Est. ₹{Number(lead.valueEstimate).toLocaleString("en-IN")}</p>}
      </button>
    </div>
  );
}

function DroppableColumn({ status, children, label, color, borderColor, count }: { status: string; children: React.ReactNode; label: string; color: string; borderColor: string; count: number }) {
  const { setNodeRef, isOver } = useDroppable({ id: `col-${status}` });
  return (
    <div ref={setNodeRef} className={`w-72 flex-shrink-0 rounded-xl border ${borderColor} ${isOver ? "bg-amber-500/5" : "bg-card/50"} transition-colors`}>
      <div className={`flex items-center justify-between px-3 py-2.5 border-b ${borderColor}`}>
        <div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${color.split(" ")[0]}`} /><span className="text-sm font-semibold">{label}</span></div>
...
export default function FranchiseeLeads() {
  ...
  const { data: leadsData, isLoading } = useQuery({ queryKey: ["franchisee-leads", search, filterStatus, filterSource], queryFn: () => fetchLeads({ search: search || "", status: filterStatus, source: filterSource, limit: "100" }) });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    qc.setQueryData(["franchisee-leads", search, filterStatus, filterSource], (old: any) => {
      if (!old) return old;
      return { ...old, data: old.data.map((l: Lead) => l.id === leadId ? { ...l, status: targetStatus as LeadStatus } : l) };
    });
  ...
  const listColumns: Column<Lead>[] = [
    ...
    { key: "status", header: "Status", cell: (l) => { const col = PIPELINE.find(p => p.status === l.status); return <span className={`text-xs px-2 py-0.5 rounded-md font-medium ${col?.color || ""}`}>{col?.label || l.status}</span>; } },
    { key: "city", header: "City", cell: (l) => <span className="text-xs text-muted-foreground">{l.city || "—"}</span> },
    { key: "value", header: "Est. Value", align: "right", cell: (l) => <span className="text-xs text-primary font-medium">{l.valueEstimate ? `₹${Number(l.valueEstimate).toLocaleString("en-IN")}` : "—"}</span> },
    { key: "followUp", header: "Follow-up", align: "center", cell: (l) => l.nextFollowUpAt ? <span className="text-[10px] text-yellow-400">{new Date(l.nextFollowUpAt).toLocaleDateString("en-IN")}</span> : <span className="text-[10px] text-muted-foreground">—</span> },
  ];
    ...
    <FranchiseeLayout>
      <div className="p-6 space-y-5">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="font-display font-bold text-2xl flex items-center gap-2"><Funnel size={22} className="text-amber-500" /> Leads</h1>
            <p className="text-muted-foreground text-sm mt-1">Manage leads in your city</p>
          </div>
          <div className="flex items-center gap-2">
            <div className="flex items-center border border-border rounded-md overflow-hidden">
              <button onClick={() => setView("kanban")} className={`px-2 py-1.5 text-xs flex items-center gap-1 ${view === "kanban" ? "bg-amber-500 text-white" : "text-muted-foreground"}`}><LayoutGrid size={12} />Kanban</button>
              <button onClick={() => setView("list")} className={`px-2 py-1.5 text-xs flex items-center gap-1 ${view === "list" ? "bg-amber-500 text-white" : "text-muted-foreground"}`}><List size={12} />List</button>
            </div>
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <Button className="bg-amber-500 text-white hover:bg-amber-500/90"><Plus size={15} className="mr-1.5" />Add Lead</Button>
              </DialogTrigger>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm text-muted-foreground">
            <span className="bg-amber-500/10 text-amber-500 px-2 py-1 rounded-md font-medium">{stats?.total ?? 0} total</span>
                ...
                {PIPELINE.map(col => {
                  ...
                  return (
                    <DroppableColumn key={col.status} status={col.status} label={col.label} color={col.color} borderColor={col.borderColor} count={items.length}>
                      {isLoading ? <div className="text-center py-8 text-muted-foreground text-sm">Loading…</div> : items.length === 0 ? <div className="text-center py-6 text-muted-foreground text-xs italic">No leads</div> : items.map(lead => <DraggableLeadCard key={lead.id} lead={lead} onClick={() => setDetailId(lead.id)} />)}
                    </DroppableColumn>
                  );
                })}
              </div>
        ...
        {view === "list" && (
          <DataTable columns={listColumns} rows={leads} isLoading={isLoading} rowKey={l => l.id} onRowClick={l => setDetailId(l.id)} emptyTitle="No leads found" emptyDescription="Try adjusting your filters or add a new lead." />
        )}
      </div>
          ...
          <div className="w-full max-w-lg bg-background h-full overflow-y-auto border-l border-border shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5"><Phone size={10} />{detail.phone}{detail.secondaryPhone && <span>· {detail.secondaryPhone}</span>}{detail.city && <span>· {detail.city}</span>}</div>
              </div>
            ...
            <div className="p-5 space-y-5">
              <div className="flex items-center gap-1 border-b border-border">
                {(["overview", "activity", "followups"] as DetailTab[]).map(tab => (
                  <button key={tab} onClick={() => setDetailTab(tab)} className={`px-3 py-2 text-sm font-medium border-b-2 transition-colors ${detailTab === tab ? "border-amber-500 text-amber-500" : "border-transparent text-muted-foreground hover:text-white"}`}>
              ...
              {detailTab === "overview" && (
                <div className="space-y-5">
                  <div className="flex items-center gap-3 flex-wrap">
                    <select value={detail.status} onChange={e => patchMut.mutate({ id: detail.id, body: { status: e.target.value as any } })} className="bg-card border border-border rounded-md px-3 py-1.5 text-sm font-medium">
                      {PIPELINE.map(p => <option key={p.status} value={p.status}>{p.label}</option>)}
                    </select>
                  ...
                  {showConvert && (
                    <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                      <h3 className="font-semibold text-sm flex items-center gap-2"><ArrowRight size={14} className="text-amber-500" />Convert Lead</h3>
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" className="bg-amber-500 text-white" onClick={() => convertMut.mutate({ id: detail.id, body: convertForm })}><CheckCircle size={13} className="mr-1.5" />Confirm</Button>
                ...
                <div className="space-y-5">
                  <div className="space-y-3">
                    {detail.activities?.map((act, i) => (
                      <div key={act.id} className="flex gap-3">
                        <div className="flex flex-col items-center"><div className="w-2 h-2 rounded-full bg-amber-500" />{i < (detail.activities?.length ?? 0) - 1 && <div className="w-px flex-1 bg-border mt-1" />}</div>
                        <div className="flex-1 pb-3">
                          <div className="flex items-center gap-2 mb-0.5"><Badge variant="outline" className="text-[10px] h-5 capitalize">{act.type}</Badge><span className="text-xs text-muted-foreground">{new Date(act.createdAt).toLocaleString("en-IN")}</span></div>
                  ...
                  <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <Button size="sm" className="bg-amber-500 text-white ml-auto" onClick={() => { activityMut.mutate({ id: detail.id, body: { type: "note", body: newNote, followUpAt: newFollowUp } }); setNewNote(""); setNewFollowUp(""); }}><Send size={13} className="mr-1.5" />Save</Button>
                    </div>
                  </div>
                </div>
              )}
              {detailTab === "followups" && (
                <div className="space-y-4">
                  <div className="bg-card border border-border rounded-xl p-4">
                    ...
                    {detail.nextFollowUpAt ? <div className="flex items-center gap-2 text-sm text-yellow-400"><Calendar size={14} />{new Date(detail.nextFollowUpAt).toLocaleString("en-IN")}</div> : <p className="text-sm text-muted-foreground italic">No follow-up scheduled</p>}
                  </div>
                  <div className="bg-card border border-border rounded-xl p-4 space-y-3">
                    ...
                    <Button size="sm" className="bg-amber-500 text-white" onClick={() => { activityMut.mutate({ id: detail.id, body: { type: "follow_up", body: "Follow-up scheduled", followUpAt: newFollowUp } }); setNewFollowUp(""); }}><Calendar size={13} className="mr-1.5" />Schedule</Button>
                  </div>
                </div>
              )}
            </div>
