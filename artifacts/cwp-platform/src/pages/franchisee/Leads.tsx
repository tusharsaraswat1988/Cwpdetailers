...
import {
  ...
  CheckCircle, UserPlus, Search, Clock, BarChart3, X, Send,
} from "lucide-react";
...
async function addActivity(id: number, body: { type: string; body: string; followUpAt?: string }) {
  const res = await fetch(`/api/leads/${id}/activities`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
...
export default function FranchiseeLeads() {
  ...
  const { data: leadsData, isLoading } = useQuery({
    queryKey: ["franchisee-leads", search, filterStatus, filterSource],
  ...
  const { data: detail } = useQuery({
    queryKey: ["franchisee-leadDetail", detailId],
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["franchisee-leads"] }); qc.invalidateQueries({ queryKey: ["franchisee-leadStats"] }); setOpen(false); toast({ title: "Lead created" }); },
  ...
  const patchMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["franchisee-leads"] }); qc.invalidateQueries({ queryKey: ["franchisee-leadStats"] }); qc.invalidateQueries({ queryKey: ["franchisee-leadDetail", detailId] }); toast({ title: "Lead updated" }); },
  });
  const convertMut = useMutation({
    mutationFn: ({ id, body }: { id: number; body: Record<string, unknown> }) => convertLead(id, body),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["franchisee-leads"] }); setShowConvert(false); setDetailId(null); toast({ title: "Lead converted" }); },
  });
  const activityMut = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["franchisee-leadDetail", detailId] }); toast({ title: "Activity added" }); },
  });
  ...
  const [convertForm, setConvertForm] = useState({ createCustomer: true, createBooking: false, createSubscription: false, serviceId: "", scheduledDate: "", amount: "", subscriptionType: "monthly" });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h1 className="font-display font-bold text-2xl flex items-center gap-2">
              <Funnel size={22} className="text-amber-500" /> Leads
            </h1>
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-3 mt-2">
                  ...
                  <Button className="w-full bg-amber-500 text-white" onClick={() => createMut.mutate({
        ...
        <div className="overflow-x-auto pb-2">
          <div className="flex gap-4 min-w-max">
            {PIPELINE.map(col => {
              ...
              return (
                <div key={col.status} className={`w-72 flex-shrink-0 rounded-xl border ${col.borderColor} bg-card/50`}>
                  <div className={`flex items-center justify-between px-3 py-2.5 border-b ${col.borderColor}`}>
                    <div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${col.color.split(" ")[0]}`} /><span className="text-sm font-semibold">{col.label}</span></div>
                    ...
                    {isLoading ? (
                      ...
                      items.map(lead => (
                        <button key={lead.id} onClick={() => setDetailId(lead.id)}
                          className="w-full text-left bg-card border border-border rounded-lg p-3 hover:border-amber-500/40 hover:shadow-sm transition-all group">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex justify-end">
          <div className="w-full max-w-lg bg-background h-full overflow-y-auto border-l border-border shadow-2xl">
            <div className="flex items-center justify-between p-5 border-b border-border">
              <div>
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground mt-0.5">
                  <Phone size={10} />{detail.phone}{detail.secondaryPhone && <span>· {detail.secondaryPhone}</span>}{detail.city && <span>· {detail.city}</span>}
                </div>
              </div>
            ...
            <div className="p-5 space-y-5">
              ...
              <div>
                <h3 className="font-semibold text-sm mb-3 flex items-center gap-2"><MessageSquare size={14} className="text-amber-500" />Activity Timeline</h3>
